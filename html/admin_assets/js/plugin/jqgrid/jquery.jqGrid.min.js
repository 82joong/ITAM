	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			beforeRequest: function() {
				// keep last status. hamt.
				if((typeof $.cookie) == 'undefined') {
					alert('$.cookie plugin Required. - hamt - ');
					return;
				}
				if(location.href.indexOf('keep=yes') < 0) {
					//$.removeCookie('jqgrid_last_postdata');
					return;
				}
				if(this.p.isFirstRequest == true) {
					this.p.isFirstRequest = false;
					var last_stat_str = $.cookie('jqgrid_last_postdata');
					if(last_stat_str) {
						var last_stat = JSON.parse(last_stat_str);

						if(last_stat.url != location.href.split('?')[0]) {
							//$.removeCookie('jqgrid_last_postdata');
						} else {
							var postdata = last_stat.data;
							var nd = this.p.postData.nd;
							postdata.nd = nd;
							this.p.postData = postdata;

							//console.log(this.p.postData);

							
							if(postdata.page > 1) {
								$(this.p.pager+' input.ui-pg-input').val(postdata.page);
							}
							$(this.p.pager+' select.ui-pg-selbox').val(postdata.rows);
						}
					}
				}
			},
		...
		addMulti = function(rowid,pos,irow,checked){
            ...
            /*
            // SmartAdmin Ver3. checkbox UI
            var v = '';
            v += '<div class="custom-control custom-checkbox">';
			v += "<input role=\"checkbox\" type=\"checkbox\""+" id=\"jqg_"+ts.p.id+"_"+rowid+"\" class=\"cbox custom-control-input\" name=\"jqg_"+ts.p.id+"_"+rowid+"\"" + (checked ? "checked=\"checked\"" : "")+"/>",
            v += '<label class="custom-control-label" for="jqg_'+ts.p.id+'_'+rowid+'"></label>';
            v += '</div>';
            */
		...
		setPager = function (pgid, tp){
			...
			if(ts.p.rowList.length >0){
				str = "<td dir='"+dir+"'>";
				str +="<select class='ui-pg-selbox form-control custom-select-sm border-info' role='listbox'>";
			...
			if(dir==="rtl") { pgl += str; }
			if(ts.p.pginput===true) { pginp= "<td dir='"+dir+"' class='form-label'>"+$.jgrid.format(ts.p.pgtext || "","<input class='ui-pg-input border-info' type='text' size='2' maxlength='7' value='0' role='textbox'/>","<span id='sp_1_"+$.jgrid.jqID(pgid)+"'></span>")+"</td>";}
			if(ts.p.pgbuttons===true) {

                var btn_class = 'btn btn-sm btn-outline-default  waves-effect waves-themed';

				var po=["first"+tp,"prev"+tp, "next"+tp,"last"+tp]; if(dir==="rtl") { po.reverse(); }
				pgl += "<td id='"+po[0]+"' class='ui-pg-button ui-corner-all'><span class='"+btn_class+"'><i class='fal fa-arrow-to-left'></i></span></td>";
				pgl += "<td id='"+po[1]+"' class='ui-pg-button ui-corner-all'><span class='"+btn_class+"'><i class='fal fa-arrow-left'></i></span></td>";
				pgl += pginp !== "" ? sep+pginp+sep:"";
				pgl += "<td id='"+po[2]+"' class='ui-pg-button ui-corner-all'><span class='"+btn_class+"'><i class='fal fa-arrow-right'></i></span></td>";
				pgl += "<td id='"+po[3]+"' class='ui-pg-button ui-corner-all'><span class='"+btn_class+"'><i class='fal fa-arrow-to-right'></i></span></td>";
			} else if (pginp !== "") { pgl += pginp; }
			if(dir==="ltr") { pgl += str; }
			pgl += "</tr></tbody></table>";
			if(ts.p.viewrecords===true) {$("td#"+pgid+"_"+ts.p.recordpos,"#"+pgcnt).append("<div dir='"+dir+"' style='text-align:"+ts.p.recordpos+"' class='ui-paging-info form-label'></div>");}
		...
		var thead = "<thead><tr class='ui-jqgrid-labels bg-primary-600' role='rowheader'>",
		tdc, idn, w, res, sort,
		...
		imgs = "<span class='s-ico' style='display:none; float:right; cursor:pointer;'>";
        imgs += "<span sort='asc' class='ui-grid-ico-sort ui-icon-asc"+iac+" ui-state-disabled ui-icon ui-icon-triangle-1-n ui-sort-"+dir+"'>";
        //imgs += "<i class='fal fa-long-arrow-alt-up'></i>";
        imgs += "</span>";

		imgs += "<span sort='desc' class='ui-grid-ico-sort ui-icon-desc"+idc+" ui-state-disabled ui-icon ui-icon-triangle-1-s ui-sort-"+dir+"'>";

        //imgs += "<i class='fal fa-long-arrow-alt-down'></i>";
        imgs += "</span>";
        imgs += "</span>";
		if(ts.p.multiSort) {
		...
		if(rdata) {
			...
			this.each(function() {
				...
				while(k < datalen) {
					...
					if(gi) {
						v = "<input role=\"checkbox\" type=\"checkbox\""+" id=\"jqg_"+t.p.id+"_"+rowid+"\" class=\"cbox23\" />";
...
$.jgrid.extend({
	...
	filterToolbar : function(p){
		p = $.extend({
			...
			resetIcon : "<i class='fal fa-times'></i>",
			searchIcon : "<span class='btnDoTriggerToolbarClass btn btn-dark btn-sm btn-icon waves-effect waves-themed'><i class='fal fa-search'></i></span>",
		...
		return this.each(function(){
			...
			$.each($t.p.colModel,function(ci){
				...
				thd = $("<div></div>"),
				stbl = $("<table class='ui-search-table' cellspacing='0'><tr><td class='ui-search-oper' valign='middle'></td><td class='ui-search-input' valign='middle'></td><td class='ui-search-clear' valign='middle'></td></tr></table>");

				if(this.attr == null) { // by hamt.
					this.attr = '';
				}
				if(this.stype == 'between') { // by hamt.
					stbl = $("<table class='ui-search-table' cellspacing='0'><tr><td class='ui-search-oper' valign='middle'></td><td class='ui-search-input' valign='middle'></td><td class='ui-search-clear' valign='middle'></td></tr><tr><td class='ui-search-oper' valign='middle'></td><td class='ui-search-input' valign='middle'></td><td class='ui-search-clear' valign='middle'></td></tr></table>");
				}
				...
				if(this.search){
					if(p.searchOperators) {
						...
						if(soptions.sopt != null && soptions.sopt.length == 1) { // by hamt. 옵션 하나만 넘기면 앞에 안나오게.
							so = soptions.sopt[0];
						}
						...
						var st = soptions.searchtitle != null ? soptions.searchtitle : (p.operandTitle != null) ? p.operandTitle : '';
						select = "<a title='"+st+"' style='' soper='"+so+"' class='soptclass' colname='"+this.name+"'>"+sot+"</a>";
					}
					if(this.stype == 'between') { // by hamt.
						select = "<a title='"+st+"' style='' soper='ge' class='soptclass' colname='"+this.name+"_from'>"+sot+"</a>";
						$('tr:eq(0) td:eq(0)', stbl).attr('colindex', ci+'_from').append(select).hide();
						select = "<a title='"+st+"' style='' soper='le' class='soptclass' colname='"+this.name+"_to'>"+sot+"</a>";
						$('tr:eq(1) td:eq(0)', stbl).attr('colindex', ci+'_to').append(select).hide();
					} else {
						$("td:eq(0)",stbl).attr("colindex",ci).append(select);
						if(soptions.sopt != null && soptions.sopt.length == 1) {
							// 서치옵션 하나면 앞에 셀렉박스 안보이게.
							$("td:eq(0)",stbl).hide();
						}
					}
					...
					if(soptions.clearSearch) {
                        ...
                        var resetClass = 'btn btn-sm btn-icon btn-default waves-effect waves-themed';
						if(this.stype == 'textlist') { // by hamt.
                            // textlist 형태는 삭제버튼 둘다 나오게
						    $("tr:eq(0) td:eq(2)",stbl).append("<div><a title='"+csv+"' class='"+resetClass+"'>"+p.resetIcon+"</a></div>");
							$("tr:eq(0) td:eq(2)",stbl).append('<div>'+p.searchIcon+'</div>');
						} else if(this.stype == 'between') { // by hamt.
                            // between 형태는 삭제버튼 둘다 나오게
						    $("tr:eq(0) td:eq(2)",stbl).append("<a title='"+csv+"' class='"+resetClass+"'>"+p.resetIcon+"</a>");
						    $("tr:eq(1) td:eq(2)",stbl).append("<a title='"+csv+"' class='"+resetClass+"'>"+p.resetIcon+"</a>");
                        } else {
                            // 일반 1개 검색창 우측 x추가
						    $("tr:eq(0) td:eq(2)",stbl).append("<a title='"+csv+"' class='"+resetClass+"'>"+p.resetIcon+"</a>");
                        }
					} else {
						$("tr:eq(0) td:eq(2)", stbl).hide();
						if(this.stype == 'between') { // by hamt
							$("tr:eq(1) td:eq(2)", stbl).hide();
						}
					}
					switch (this.stype)
					{
					case "select":
						...
						break;
					case "text":
					case "textlist": // by hamt.
					case "between":   // by hamt.
						...
						if(this.stype == 'text') {
							$("tr:eq(0) td:eq(1)",stbl).append("<input type='text' style='width:100%;' name='"+(cm.index || cm.name)+"' id='gs_"+cm.name+"' value='"+df+"' "+this.attr+" placeholder='Search Text' aria-label='Search Text' aria-describedby='button-addon5' />");
						} else if(this.stype == 'between') { // by hamt.
							$("tr:eq(0) td:eq(1)",stbl).append("<input type='text' placeholder='From' style='width:100%;' name='"+(cm.index || cm.name)+"_from' id='gs_"+cm.name+"_from' value='"+df+"' "+this.attr+" />");
							$("tr:eq(1) td:eq(1)",stbl).append("<input type='text' placeholder='To' style='width:100%;' name='"+(cm.index || cm.name)+"_to' id='gs_"+cm.name+"_to' value='"+df+"' "+this.attr+" />");
						} else if(this.stype == 'textlist') { // by hamt.
                            form_tag = 'textarea';
							$("tr:eq(0) td:eq(1)",stbl).append("<textarea style='width:100%;min-width:50px;display:block;height:50px;' name='"+(cm.index || cm.name)+"' id='gs_"+cm.name+"' value='"+df+"' "+this.attr+" />");
                        }

						$(thd).append(stbl);

						if(soptions.attr) {$(form_tag,thd).attr(soptions.attr);};
						//$.jgrid.bindEv.call($t, $("input",thd)[0], soptions);
						$.jgrid.bindEv.call($t, $(form_tag,thd), soptions);
						if(p.autosearch===true && form_tag != 'textarea'){
							if(p.searchOnEnter) {
								...
								// todo. apply search input script.
            ...
            $(".btnDoTriggerToolbarClass",tr).click(function(e){
					triggerToolbar();
            })
			$(".clearsearchclass",tr).click(function(e){
				...
				} else if($t.p.colModel[coli].stype === "textlist") {
					$("td.ui-search-input textarea", ptr).val( dval );
				} else {
	...
	navGrid : function (elem, o, pEdit,pAdd,pDel,pSearch, pView) {
		...
		return this.each(function() {
			...
			for(i = 0; i<clone; i++) {
				...
				if (o.refresh) {
					tbd = $("<td class='ui-pg-button ui-corner-all'></td>");
					$(tbd).append("<div class='ui-pg-div btn btn-sm btn-success'><i class='"+o.refreshicon+"'></i>"+o.refreshtext+"</div>");
					$("tr",navtbl).append(tbd);
					$(tbd,navtbl)
				...
				if(cnt > 0) {
					...
					if(o.keys===true) {
						$(ind).bind("keydown",function(e) {
							...
							if (e.keyCode === 13) {
								var ta = e.target;
								if(ta.tagName === 'TEXTAREA') { if(e.shiftKey) return true; e.preventDefault(); }
