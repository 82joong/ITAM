<?php
/**
 * MakeShop Platinum
 *
 * @package MakeShop Platinum
 * @author ROOT TF Team
 * @copyright Copyright (c) 2016, KOREACENTER.COM, Inc. (http://makeshop.co.kr/)
 */
defined('BASEPATH') or exit('No direct script access allowed');

/**
 * Company_tb Model
 *
 * @package MakeShop Platinum
 * @author ROOT TF Team
 * @created
 */

class Company_tb_model extends MY_Model
{

    protected $pk = 'c_id';

    protected $emptycheck_keys = array(
        'c_code' => 'c_code 값이 누락되었습니다.',
        'c_name' => 'c_name 값이 누락되었습니다.',
        /*
        'c_tel' => 'c_tel 값이 누락되었습니다.',
        'c_biz_number' => 'c_biz_number 값이 누락되었습니다.',
        'c_biz_owner' => 'c_biz_owner 값이 누락되었습니다.',
        'c_is_active' => 'c_is_active 값이 누락되었습니다.',
        'a_created_at' => 'a_created_at 값이 누락되었습니다.',
        'a_updated_at' => 'a_updated_at 값이 누락되었습니다.',
        */
    );

    protected $enumcheck_keys = array(
    );

    protected $code_text_map = array();

    public function __construct()
    {
        parent::__construct();
        $this->db_name = _SHOP_INFO_DATABASE_;
        $this->table   = strtolower(substr(__CLASS__, 0, -6));
        $this->fields  = $this->db->list_fields($this->table);
    }


	public static $is_active = array(
			'YES'	=> 'YES',
			'NO'	=> 'NO',
	);
    public function getIsActiveMap() {
        return self::$is_active;
    }


    protected function __filter($params)
    {
        $params['c_created_at'] = date('Y-m-d H:i:s');
        $params['c_updated_at'] = date('Y-m-d H:i:s');
        return $params;
    }

    protected function __validate($params)
    {
        $success = parent::__validate($params);
        if ($success === true) {
        }
        return $success;
    }

    protected function __updateFilter($params)
    {
        $params['a_updated_at'] = date('Y-m-d H:i:s');
        return $params;
    }

    protected function __updateValidate($params)
    {
        $success = parent::__updateValidate($params);
        if ($success === true) {
        }
        return $success;
    }
}
