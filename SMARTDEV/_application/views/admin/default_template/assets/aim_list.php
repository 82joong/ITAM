    ...
    <table id="data-table" class="table table-sm table-striped table-bordered table-hover" style="width:100%">
        <thead class="bg-warning-200">
            <tr>
                ...
                <th data-name="idrac_ip" data-type="text" data-op="eq">iDrac IP</th>
                ...
                <th data-name="aim_id">AIM ID.</th>
                <th data-name="idrac_aim_id">iDrac AIM ID.</th>
...
$(document).ready(function() {
    ...
    var table = $('#data-table').DataTable({
        ...
        "columns": [
            ...
            {"data": "am_rack_code"},
            {"data": "ip_memo"},
            {"data": "aim_id", "visible": false},
            {"data": "idrac_aim_id", "visible": false},
            {"data": null},
        ],
        ...
        "columnDefs": [
            { "className": "fs-nano text-center", "targets": "_all" },
            {
                ...
                "render": function(data, type, full, meta) {
                    ...
                    // Delete 
                    var base_url = 'javascript:delRow(\''+data.ip_id+'\', \''+data.aim_id+'\', \''+data.idrac_ip+'\', \''+data.idrac_aim_id+'\');';
    ...
    $('#btn-save').click(function(e) {
        ...
        if( ! _modal.find('#am_id').val() || ! _modal.find('#am_vmware_name').val() ) {
        ...
        params += '&request=ajax';
        $.post('/<?=SHOP_INFO_ADMIN_DIR?>/assets/pre_aim_process', params, function(res) {
            if(res.is_success) {
...
function editRow(id, ip, ipc_id) {
    ...
    var params = {'ip_id':id, 'ip': ip, 'ipc_id': ipc_id, 'view_type':'aim'};
...
function delRow(id, aim_id, idrac_ip, idrac_aim_id) {
    ...
    Swal.fire({
        ...
        if (result.value) {
            var params = {'ip_id':id, 'aim_id':aim_id, 'idrac_ip':idrac_ip, 'idrac_aim_id':idrac_aim_id, 'mode':'delete'};
            $.post('/<?=SHOP_INFO_ADMIN_DIR?>/assets/del_aim_process', params, function(res) {
                if(res.is_success) {
