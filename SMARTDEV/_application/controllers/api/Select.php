...
class Select extends REST_Controller {
    ...
    public function account_post() {
        ...
        $data = $this->daoudata->getData('ACCOUNT');
        //echo print_r($data);

        $rows = array();
        foreach($data as $k=>$d) {
            if(strpos($d['koName'], $req['q']) !== FALSE) {
                $rows[] = array(
                    'id' => $d['koName'],
                    'name' => $d['koName'],
        ...
        $json_data['is_success'] = true;
        $json_data['data'] = array( 'total_count' => sizeof($rows));


        if( sizeof($rows) < 1 ) {
            $json_data['data']['items'] = array();
        }else {
            $json_data['data']['items'] = $rows;
        }
        echo json_encode($json_data);
        return;
    }
    ...
    public function dept_post() {
        ...
        $data = $this->daoudata->getData('DEPT');
        //echo print_r($data);
        //exit;
